...
module.exports = {
  meta: {
    namse: 'eslint-plugin-twig',
    version: '0.0.1',
  },
  processors: {
    '.twig': {
      preprocess(text, filename) {
        // Escaping '{{' and '}}' that are not part of a variable expression
        ...
        sanitizedText = sanitizedText.replace(/\{\{(.*?)\}\}/g, (str) => {
          ...
          const uniqueID = uniqueCounter.toString().padStart(replacementLength, '0');
        ...
        sanitizedText = sanitizedText.replace(/\{%([^\r\n]*?)%\}/g, (str) => {
          const replacementLength = str.length - 2; // Adjusting for the length of '{%' and '%}'
          uniqueCounter++;
          const uniqueID = uniqueCounter.toString().padStart(replacementLength - 2, '0');
        ...
        fileContentMap.set(filename, { original: text, sanitized: sanitizedText });
      ...
      postprocess(messages, filename) {
        ...
        flattenedMessages.forEach((message) => {
          if (message.fix) {
            ...
            const uniqueIDs = fixText.match(/(\/\d+\/)|(\/\/\d+\/\/)|(\/\*\d+\*\/)/g);
            ...
            if (uniqueIDs) {
              uniqueIDs.forEach((uniqueIDTag) => {
                ...
                while ((match = sanitizedRegex.exec(sanitized)) !== null) {
                  ...
                  const originalContent = original.substring(startIndex, endIndex); // Extract original content from the same position
